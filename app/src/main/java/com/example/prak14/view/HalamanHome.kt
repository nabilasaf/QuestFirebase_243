...
import com.example.prak14.view.route.DestinasiHome
import com.example.prak14.viewmodel.HomeViewModel
import com.example.prak14.viewmodel.PenyediaViewModel
import com.example.prak14.viewmodel.StatusUiSiswa
...
fun HomeBody(
    ...
    Column(
        ...
        when (statusUiSiswa) {
            ...
            is StatusUiSiswa.Success -> DaftarSiswaItemView(
                daftarSiswa = statusUiSiswa.siswa,
                onSiswaClick = {onSiswaClick(it.id.toInt())},
                modifier = modifier
            )
            is StatusUiSiswa.Error -> OnError(
                retryAction,
...
@Composable
fun DaftarSiswaItemView(
    daftarSiswa: List<Siswa>,
    onSiswaClick: (Siswa) -> Unit,
    modifier: Modifier = Modifier
) {
    LazyColumn(modifier = modifier) {
        items(items = daftarSiswa, key = { it.id }) { person ->
            SiswaCard(
                siswa = person,
                modifier = Modifier
